...
void enem_shoot()
{
    ...
    if (gamestate == 21)
    {
        ...
        {
            if (enem6hp <= 120)
            { // Spread shot when HP <= 20% of original (50 * 0.2 = 10)
                int num_bullets = 3;
                double angles[3] = {-0.261799, 0.0, 0.261799}; // -15°, 0°, +15° in radians
                double bullet_speed = 30.0;                    // Match existing bullet speed
                int fired = 0;
                for (int i = 0; i < MAX_BULLETS && fired < num_bullets; i++)
                {
                    if (!e6bullet_active[i])
                    {
                        ...
                        e6bullet_y[i] = enem6_y + 70;                        // Center of sprite
                        e6bullet_vx[i] = -bullet_speed * cos(angles[fired]); // Leftward x-velocity
                        e6bullet_vy[i] = bullet_speed * sin(angles[fired]);  // y-velocity for spread
                        iSetSpritePosition(&e6bulsprite[i], e6bullet_x[i], e6bullet_y[i]);
                        e6bullet_active[i] = 1;
                        fired++;
                    }
            ...
            { // Normal single bullet
                for (int i = 0; i < MAX_BULLETS; i++)
                {
                    if (!e6bullet_active[i])
                    {
                        ...
                        e6bullet_vx[i] = -30.0; // Leftward, same as original
                        e6bullet_vy[i] = 0.0;   // No y-velocity
                        iSetSpritePosition(&e6bulsprite[i], e6bullet_x[i], e6bullet_y[i]);
                        e6bullet_active[i] = 1;
                        break;
                    }
...
void updateEnemy()
{
    ...
    if (gamestate == 21)
    {
        ...
        // Enemy 6 Management
        if (!enem6_active && !enem6_exploding && !enem6_spawned && scorenumber >= 2000)
        {
            enem6_active = true;
