...
void resetGame()
{
    ...
    // Reset Enemy 6
    // In resetGame function, replace the Enemy 6 reset block
// Reset Enemy 6
enem6_active = (gamestate == 23); // Activate immediately in Boss Mode
enem6_x = SCREEN_WIDTH - 370;
enem6_y = SCREEN_HEIGHT - 310; // Start within screen bounds
...
enem6hp = 300;
bosshp = 600;
if (gamestate == 21) { enem6_spawned = false; }
else if (gamestate == 23) { enem6_spawned = true; } // Mark as spawned
        ...
        if (gamestate == 21 || gamestate == 23)
{
    if (enem6_active && !enem6_exploding)
    {
        ...
        else if (gamestate == 23 && bosshp <= 200) // 33.33% of 600
        {
            int num_bullets = 3;
            double angles[3] = {-0.261799, 0.0, 0.261799}; // -15°, 0°, +15° in radians
            double bullet_speed = 30.0;
        ...
        else if (gamestate == 23 && bosshp <= 400) // 66.67% of 600
        {
            int num_bullets = 5;
...
if (gamestate == 21 || gamestate == 23)
{
    if (!enem6_active && !enem6_exploding && !enem6_spawned && gamestate == 21 && scorenumber >= 2000) // Restrict score check to gamestate == 21
    {
        enem6_active = true;
        enem6_spawned = true;
        enem6_x = SCREEN_WIDTH - 370;
...
void bossexplosion()
{
    ...
    if (enem6_exploding)
    {
        ...
        else
        {
            ...
            iSetSpritePosition(&enem6, enem6_x, enem6_y);
            prev_gamestate = gamestate;
            gamestate = 211; // Transition to game over
        }
    }
}
//=============================================================================
        ...
        if (enem6_active && !enem6_exploding)
        {
            for (int i = 0; i < MAX_BULLETS; i++)
            {
                if (bullet_active[i])
                {
                    ...
                    {
                        bullet_active[i] = 0;
                        if (gamestate == 21) enem6hp--;
                        else if (gamestate == 23) bosshp --;
                        if (gamestate == 23)
                        {
                            scorenumber += 100;
                            sprintf(scoretext, "%d", scorenumber);
...
void sound_manage()
{
    if (gamestate == 1)
    {
        if (mainidx != -1) iStopSound(mainidx); // Stop mainidx if playing
        if (sound_check == 0) // Respect sound_check
            homeidx = iPlaySound("assets/sounds/menubg.wav", true, 70);
    }
    else if (gamestate == 21 || gamestate == 23)
    {
        if (homeidx != -1) iPauseSound(homeidx);
        if (mainidx != -1) iStopSound(mainidx); // Stop previous mainidx
        if (sound_check == 0) // Respect sound_check
...
void mainpage1()
{
    ...
    if (gamestate == 23) enem6_active = true;
    // Shield Display
    if (shield_active)
    {
...
void iDraw()
{
    ...
    mainpage1();
        ...
        break;
    case 211:
        if (sound_check == 0 && mainidx != -1) iPauseSound(mainidx); // Pause mainidx
...
void iKeyboard(unsigned char key, int state)
{
    ...
    if (homeidx != -1) iStopSound(homeidx);
    if (mainidx != -1) iStopSound(mainidx);
    iCloseWindow();
    break;
...
void timer()
{
    ...
    // In timer function, replace the survival score timer block
if (gamestate == 21 || gamestate == 23) // Changed from gamestate == 21
{
